...
### Modifications matérielles sur le scanner
  ...
  9. Assembler les morceaux de l’ensemble optique;
  > Certaines parties de l’ensemble optique se vissent et dévissent ensemble; d’autres parties sont tenus par trois petites vis autour, qu’il faut enlever et remettre avec une mini clé Allen qui se trouve dans un petit sac en plastique.
  8. Consulter la charte de dôme d’éclairage pour savoir lequel est approprié; retirer et remplacer au besoin;
  > Parfois, il faut également changer le bloc de métal attaché au dôme; pour ce faire, il faut utiliser la clé Allen 2.5 (vert pâle).
  9. Revisser l’ensemble optique.
...
4. Mettre l’ouverture de l’objectif à la plus grande valeur possible (donc le chiffre le plus petit);
> Il faut dévisser légèrement la petite vis noire sur l’objectif et aligner la ligne blanche avec la valeur désirée.
6. Dans le mandrin, insérer une sphère de pose de taille appropriée;
> La sphère de pose ne doit pas dépasser le cadre de l’image; s’assurer qu’elle est grosso-modo au centre de l’image; sinon, ajuster la hauteur de la caméra avec les deux grosses vis noires vis-à-vis les colonnes (étiquetées *Translation vers le haut/bas*).
7. Une fois la sphère de pose relativement bien centrée, retirer du mandrin puis mettre le support pour matrice de calibration; il faut choisir la matrice appropriée selon la configuration choisie;
>  L’information est indiquée sur la page présentement ouverte, à la ligne *Dot Pitch Target*, en millimètres.
8. S’assurer que la cible, une fois dans son socle, est elle aussi grosso modo au centre de l’image verticalement, mais cette fois-ci en ajustant la hauteur du socle et non celle de la caméra;
> Il se peut que vous ayez besoin d’éloigner ou de rapprocher la caméra pour voir clairement la cible, dans ce cas utiliser les boutons sur le contrôleur physique du DISC3D.
9. Ajuster le *Exposure Time* et *Gain* de sorte que les points blancs de la cible paraissent en vert très pâle et soient légèrement surexposés;
10. Ajuster la position de la caméra avec les boutons du contrôleur physique de sorte à maximiser l’indicateur de netteté *Sharpness indicator*, accessible sur le côté droit de l’application;
11. Dévisser le petit bras du bloc noir situé au bas du rail (celui où il est écrit *Ajuster sur le point focal*), et le déplacer jusqu’à ce que la petite lumière orange qui y est accrochée soit à l’extrémité droite du morceau rouge (où il est écrit *Point focal*), juste assez pour s’allumer, puis reculer légèrement tout juste pour que la lumière orange s’éteigne;
12. Revisser le petit bras;
13. Cliquer sur *Done*, puis sur *Yes* si on vous demande si vous avez changé le système optique.
14. Ajuster l’ouverture de l’objectif pour qu’elle soit à son plus petit (donc le plus grand chiffre);
...
15. À droite du bouton de sélection, deux boutons à droite du bouton avec une flèche, sélectionner *Resize region*;
...
23. Menu **Tools**, *Model*, *Clean model* :
    ...
    - *Level* : ***99***
    - Ok
24. Cliquer sur l’outil de sélection et entourer les bouts à enlever (ex. l’aiguille) et supprimer;
...
4. Ouvrir le fichier *(...)_HIGH.ply* avec Cloud Compare;
...
11. Aller dans le dossier *colmap-x64-windows-cuda* qui se trouve dans le dossier racine des projets de scan;
12. Clic-droit sur le fichier *COLMAP.bat*, *Run as administrator*;
13. Menu **File**, *Import model from* :
    - Sélectionner le fichier *(...)_LOW.ply* précédemment créé;
14. Menu **File**, *Export model*;
    - Créer un dossier *colmap* et sélectionner;
15. Fermer Colmap.

### Dans le dossier du projet

16. Aller dans le dossier *colmap*;
17. Copier le fichier *points3D.bin*;
18. Aller dans le dossier *sparse*, *0*
19. Coller-remplacer le fichier *points3D.bin*.

# 9. RENOMMAGE DES PHOTOS POUR POSTSHOT

1. Créer un dossier *edof_exported_renamed*;
...
4. Menu **File**, *Export Scene Rdnc Field* :
    ...
    - *Save*.
5. Sauvegarder le projet Postshot.
6. Fermer Postshot.
